...
import aynl.net.entity.domain.AppException;
import aynl.net.utils.util.LogUtil;
...
public class HttpRequestLogAspect {
    ...
    private static org.slf4j.Logger log = LoggerFactory.getLogger(HttpRequestLogAspect.class);
     ...
     * @throws
     */
    @Around("aynl.net.web.aop.PointCutsHttp.httpLog()")
    public Object logProcessInfo(ProceedingJoinPoint pj) throws Throwable {
        ...
        try {
            ...
            if (args.length > 0 && args[0].getClass().toString().equals("class org.apache.catalina.connector.RequestFacade"))
                log(start, signature.getName(), targetClass.getName(), null, true, gson.toJson(ret));
            else
    ...
    private void log(long start, String interfaceName, String className, String paramInfo, boolean isNormal, String msg) {
        LogUtil.HTTPREQUEST_LOGGER.info(String.format("%s %s %s %s %s %s", interfaceName, className, System.currentTimeMillis() - start, isNormal ? "OK" : "ERROR", paramInfo, msg));
    }
}
